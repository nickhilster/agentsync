...
### 6.3 Concurrent Agent Safety Protocol

When two or more agents work on the codebase simultaneously, conflicts are prevented through two complementary mechanisms:

#### File Territory Partitioning
Before activating any developer agent, the Orchestrator assigns a `file_scope` — the explicit list of files/directories that task will touch. **No two active tasks may have overlapping file scopes.** A task is held in a pending state until its entire territory is free.

Task list entries must include:
```
Task ID: T-[N]
Description: [What to build]
Assigned to: [Agent name]
file_scope: [src/components/Login.tsx, src/api/auth.ts]   ← REQUIRED
git_branch: agent/task-[N]                                ← REQUIRED
Status: pending / active / qa / complete
```

#### Git Branch Isolation
Every developer agent works on a dedicated branch `agent/task-{id}`. The Orchestrator merges to the main dev branch only after QA passes:
```
1. Orchestrator creates branch: agent/task-{id}
2. Developer agent works exclusively on that branch
3. Evidence Collector validates the branch in isolation
4. PASS → Orchestrator merges to dev branch, releases file_scope territory
5. FAIL → Branch is discarded for retry; territory remains locked to the task
```

This enables true parallel execution: two developers can work concurrently as long as their `file_scope` sets are disjoint. External agents (e.g., Codex, other Claude Code sessions) must claim territory via the same mechanism before touching any files.

---

### 6.4 Parallel Build Tracks
